...
//dichiaro quanta trasparenza usare all'inizio
float mixValue = 0.2f;
...
int main() {
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "combinazione di due immagini", NULL, NULL);
	...
	// ------------------------------------
	Shader ourShader("Capitolo_1/esercizi_texture/4.1.shader_texture_esercizio1.vs", "Capitolo_1/esercizi_texture/4.shader_texture_3.fs");
	...
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(6 * sizeof(float)));// attributo delle coordinate vertice
...
void processInput(GLFWwindow* window)
{
	if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)// chiudo finestra se premo esc
		glfwSetWindowShouldClose(window, true);

	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)// gestico freccia in su
	{
		mixValue += 0.0001f; // change this value accordingly (might be too slow or too fast based on system hardware)
		if (mixValue >= 1.0f)// non faccio sforare il valore 1.0 a mixValue
			mixValue = 1.0f;
	}
	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)// gestisco freccia in giù
	{
		mixValue -= 0.0001f; // change this value accordingly (might be too slow or too fast based on system hardware)
		if (mixValue <= 0.0f)
			mixValue = 0.0f;
