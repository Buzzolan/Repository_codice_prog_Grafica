...
int main() {
   ...
   // ------------------------------------
	Shader ourShader("Capitolo_1/Esercizi Shader/3.3.shader.vs", "Capitolo_1/Esercizi Shader/3.3.shaderunifrom.fs");
	...
	*/
	...
	while (!glfwWindowShouldClose(window))// controllo che la finestra non sia stata chiusa ogni volta che inizio il loop
	{
		...
		glClearColor(0.0f, 0.0f, 1.0f, 1.0f);//rosso, verde, blu
		...
		ourShader.setFloat("ourColorR", greenValue);
		ourShader.setFloat("ourColorG", greenValue);
		ourShader.setFloat("ourColorB", greenValue);
