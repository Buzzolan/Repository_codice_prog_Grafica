...
int main() {
	...
	// l'output di uno shader è l'input per quello successiv
	...
	while (!glfwWindowShouldClose(window))// controllo che la finestra non sia stata chiusa ogni volta che inizio il loop
	{
		...
		glUseProgram(shaderProgramOrange); //lo vado ad attivare
		glBindVertexArray(VAOs[0]);
		glDrawArrays(GL_TRIANGLES, 0, 3);

		glUseProgram(shaderProgramYellow); //lo vado ad attivare
